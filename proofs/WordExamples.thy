...
lemma "(((ucast a) + (ucast b)) :: 'p::len word) = (ucast b + ucast a)" for a :: \<open>'q::len word\<close> and b :: \<open>'r::len word\<close> using add.commute by simp

lemma "(((ucast a) + (ucast a)) :: 'p::len word) = ((2 * (ucast a)) :: 'p::len word)" for a :: \<open>'q::len word\<close> by auto

value "(sint (word_of_int (-1) :: 3 word)) + 10 ::int"

lemma bw_is_word: "bw p a = uint (word_of_int (a) :: 'p::len word)"if "p = LENGTH('p)" using bw_def uint_word_of_int by (metis that)


lemma "bw q (or (bw p a) (bw p b)) = or (bw p a) (bw p b)" if "q \<ge> p"
by (metis bw_def reduce_mod take_bit_eq_mod take_bit_or that) 

lemma add_as_xor_and[simp]: "\<forall> p :: nat. \<forall> a b :: int.  bw p a + bw p b = xor (bw p a) (bw p b) + 2 * (and (bw p a) (bw p b))"
...
text \<open>x + (x | (0 − x ) ) \<Rightarrow> x & (x + sext(1))\<close>
lemma hydra_57531: "bw p ((bw p x) + (or (bw p x) (bw p (0 - bw p x)))) = bw p (and (bw p x) (bw p (bw p x + bw p (-1))))"
text \<open>try could not find a proof\<close>
(* using add_as_xor_and try *)

value "or (1 :: 4 word) (ucast (2 :: 3 word))"
